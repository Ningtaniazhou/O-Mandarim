...
import styles from "./beijing-curtain-scene.module.css";
...
export default function BeijingCurtainScene({
  title,
  exterior,
  soundEnabled,
  children,
  onDepart,
}: BeijingCurtainSceneProps) {
  ...
  const ensureAudio = useCallback(async () => {
    ...
    masterGain.gain.value = soundEnabled ? 0.56 : 0;
    ...
    const streetSource = createLoop(context, makeNoiseBuffer(context, 3.4, 0.86), "lowpass", 1180, streetGain);
    const crowdSource = createLoop(context, makeNoiseBuffer(context, 4.7, 0.94), "bandpass", 480, crowdGain);
  ...
  const handlePointerDown = async (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const dragRange = Math.max(150, (stageRef.current?.clientWidth ?? 700) * 0.36);
  ...
  useEffect(() => {
    ...
    engine.masterGain.gain.setTargetAtTime(soundEnabled ? 0.56 : 0, now, 0.04);
  }, [soundEnabled]);
    ...
    return () => {
      ...
      engine.sources.forEach((source) => {
        try {
          ...
          // Development remounts may already have stopped a source.
        }
      });
      try {
        engine.lfo.stop();
      } catch {
        // Development remounts may already have stopped the oscillator.
      }
